...
For the source of the video data, Xilinx provides the VDMA IP. The VDMA reads an image stored in the FPGA's DDR memory, and outputs it as an AXI4-Stream video.\ For simplicity, the VDMA will always output the same image, so that the video will be static, however, it can be configuredd to output video as well.
